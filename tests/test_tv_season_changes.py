...
from tminidb.changes.tv_season.models import TvSeasonChangesModel
...
CHANGE_LOGS = [
    pytest.param("364732", 364732, None, None, id="season edited recently"),
    pytest.param(
        "unknown_999999999",
        999999999,
        None,
        None,
        id="season id no season has",
    ),
]
...
# TODO: Validate
class TvSeasonChangesTest(RecordedEndpoint):
...
def download_change_log(
    ...
    if start_date is not None and end_date is not None:
        return client.tv_season_changes.download_merged(season_id, start_date, end_date)
    return client.tv_season_changes.download(season_id, start_date=start_date)


# TODO: Validate
@pytest.mark.parametrize(
    ("name", "season_id", "start_date", "end_date"),
    CHANGE_LOGS,
)
def test_download(
    client: TMiniDB,
    ...
    TvSeasonChangesTest.download_test(
        name,
        lambda: download_change_log(client, season_id, start_date, end_date),
    )
...
def test_parse(client: TMiniDB, name: str) -> None:
    change_log = client.tv_season_changes.load(
        TvSeasonChangesTest.recorded_content(name),
    )
