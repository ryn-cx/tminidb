...
def test_download(
    ...
    TvEpisodeTest.download_test(
        f"{series_id}_{season_number}_{episode_number}",
        lambda: client.tv_episode.download(series_id, season_number, episode_number),
    )
...
def test_parse(
    ...
    episode = client.tv_episode.load(
        TvEpisodeTest.recorded_content(
            f"{series_id}_{season_number}_{episode_number}",
        ),
    )
