...
# TODO: Validate
class TvEpisodeDetailsTest(RecordedEndpoint):
...
def test_download(
    ...
    TvEpisodeDetailsTest.download_test(
        f"{series_id}_{season_number}_{episode_number}",
        lambda: client.tv_episode.details.download(
            series_id,
            season_number,
            episode_number,
        ),
...
def test_parse(
    ...
    episode = client.tv_episode.details.load(
        TvEpisodeDetailsTest.recorded_content(
            f"{series_id}_{season_number}_{episode_number}",
        ),
    )
    assert episode.season_number == season_number
    assert episode.episode_number == episode_number
...
def test_download_invalid(
    ...
    TvEpisodeDetailsTest.error_test(
        f"{series_id}_{season_number}_{episode_number}",
        lambda: client.tv_episode.details.download(
            series_id,
            season_number,
